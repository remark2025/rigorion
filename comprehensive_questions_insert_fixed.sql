-- Comprehensive SAT Questions with Interactive Solutions (FIXED)
...
INSERT INTO public.questions (
  ...
  'A', 
  'Choice A maintains the formal, academic tone appropriate for discussing research. The other choices use informal language that does not match the context.',
...
(gen_random_uuid(), (SELECT id FROM questions WHERE public_id = 'WRITE-GRAMMAR-001'), 2, 'Evaluate formality levels', 'Compare the formality of each choice', 'comparison', 'A: formal, B: informal ("ended up"), C: casual ("got around to"), D: implies difficulty ("managed to")', 'Consider which phrases you would use in academic writing', 'Choice evaluation', 'A is most formal'),
